...
class Filing:
    ...
    def _get_statement(self, statement_short_names):
        ...
        if FILING_SUMMARY_FILE in self.documents:
            ...
            report = filing_summary_xml.find_all('report')
            #print(report)

            '''for report in report:
                childTag = report.find('menucategory')
                if childTag and childTag.text == 'Statements':
                    #print(report) 
                    pass
                # do stuff'''
            ...
            if len(statement_names) == 0:
                filename = self.get_html_file_name_fuzzy(filing_summary_xml, 'income statement')
                statement_names += [(short_name, filename)]
    ...
    @staticmethod
    def get_html_file_name_fuzzy(filing_summary_xml, report_short_name_match):
        '''
        Return the HtmlFileName (FILENAME) of the Report in FilingSummary.xml
        (filing_summary_xml) with ShortName in lowercase matching report_short_name
        e.g.
        ...
        print('start get html file name fuzzy')

        reports = filing_summary_xml.find_all('report')
        short_names_list = []
        for report in reports:
            if report.find('menucategory') is not None:
                if report.find('menucategory').get_text() == 'Statements':
                    short_name = report.find('shortname')
                    if short_name is None:
                        print('The following report has no ShortName element')
                        #print(report)
        ...
        #exclude_list = ['comprehensive','Comprehensive']
        ...
        for matchTuple in matchRatios:
            ...
            for report in reports:
                ...
                if report_name == short_name.get_text().lower():
                    filename = report.find('htmlfilename').get_text()
                    print(filename)
                    return filename
