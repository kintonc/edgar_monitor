...
class FinancialReport_lite:
    '''
    Models financial reports from an edgar filing
    financial elements are stored in a map to retain flexibility
    '''
    def __init__(self, company, date_filed=None, reports=[]):
        '''
...
def get_financial_report_lite(company, financial_html_text, date_filed=''):
    '''
    ...
    :param_optional date_filed: datetime representing ACCEPTANCE-DATETIME of Filing

    '''
    financial_info = _process_financial_info(financial_html_text)

    # if _process_financial_info has an error, we return False.
    if financial_info == False:
        return False

    financial_report = FinancialReport_lite(company, reports=financial_info)
...
def _process_financial_info(financial_html_text):
    ...
    # if _get_statement_meta_data returns an error, return False
    if dates is False and period_units is False and unit_text is False:
        return False

    for i, date in enumerate(dates):
        try:
            # use Datetime.Parser instead of strptime, as it is more versatile
    ...
    for row_num, row in enumerate(rows):
        ...
        for index, info in enumerate(data):
            ...
            if processed_financial_value is not None:
                ...
                # If there is an error, return False. IndexError usually means this is NOT a (common) financial statement.
                # (ex: maybe it is "Income Tax statement")
                ...
                if 'tl' in class_list:
                    ...
                    for i in range(repeat):
                        if is_snapshot:
                            ...
                            # If this fails, it's because the period (normally ex: "3 months") does not contain a digit
                            # which means we are looking at wrong statement, so return False up the stack
                            # so we can look at next statement.
                            try:
                                period_units.append(_process_period(info_text))
                            except ValueError as e:
                                if str(e).startswith('invalid literal for int() with base 10:'):
                                    print(e)
                                    #raise InvalidLiteralError('invalid literal for int() with base 10')
